...
integers, find a continuous sub-array that adds to a given number S and 
...
Note:- You have to return an ArrayList consisting of two elements left 
and right. In case no such subarray exists return an array consisting of 
element -1.
...
If no such subarray is found, return an array consisting of only one 
element that is -1.
